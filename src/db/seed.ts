...
async function seed() {
  ...
  try {
    ...
    await client.query(
      'TRUNCATE mission_question_options, mission_questions, missions, trails, shop_items RESTART IDENTITY CASCADE'
    )
    ...
    for (const trail of trails) {
      const trailRow = await client.query<{ id: number }>(
        `INSERT INTO trails (slug, title, short_title, order_index)
         VALUES ($1, $2, $3, $4) RETURNING id`,
        [slugify(trail.short_title), trail.title, trail.short_title, trail.order_index]
      )
      const trailId = firstRow(trailRow).id

      for (const mission of trail.missions) {
        const missionRow = await client.query<{ id: number }>(
          `INSERT INTO missions (trail_id, slug, title, emblem, theory, has_minigame, summary, bibliography, faqs, order_index)
