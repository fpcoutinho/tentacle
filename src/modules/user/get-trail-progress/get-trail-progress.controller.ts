...
import { dto } from './get-trail-progress.dto.ts'
import { service } from './get-trail-progress.service.ts'

export async function getTrailProgress(req: Request, res: Response): Promise<void> {
  if (!req.user) {
  ...
  const result = await service.execute(params.trailId, req.user.id)
